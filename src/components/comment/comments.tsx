...
const Comments = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && commentToEdit) {
        await api.put(`/ratings/update/${commentToEdit.ratingId}`, rateDto);
      } else {
        await api.post('/ratings/add-or-update', rateDto);
      }
  ...
  const handleDelete = async (ratingId: number, userId: number) => {
    ...
    try {
      ...
      await api.delete(`/ratings/delete/${ratingId}`);
  ...
  return (
    <div className={classes.container}>
      ...
      {isLoggedIn() && (!hasRated || (hasRated && !hasCommented)) || isEditing ? (
        <>
