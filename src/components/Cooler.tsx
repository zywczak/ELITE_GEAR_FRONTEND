...
const CoolerComponent: React.FC<CoolerComponentProps> = ({ cooler }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Sending data:', JSON.stringify(updatedCooler));
    try {
      const response = await fetch(`http://localhost:8080/cooler/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(updatedCooler),
      });
  
      if (response.ok) {
        alert('Cooler data updated successfully!');
        setIsEditing(false);
        window.location.reload();
      } else {
        const errorMsg = await response.text();
        console.error('Error:', errorMsg);
        alert('Failed to update cooler data: ' + errorMsg);
      }
    } catch (error) {
    ...
    <>
      <div className={classes.container}>
        {!isEditingPhotos ? (
          ...
          {cooler.photos.map((photoUrl: string, index: number) => (
            <div key={index}>
              <input
                type="checkbox"
